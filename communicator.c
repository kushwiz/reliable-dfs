  ...
  while(currentCommand!=QUIT) {  // main accept() loop
	...
	if( i == 0 )
	{
	  ...
	  if ((nbytes = recv(i, buf, sizeof buf, 0)) <= 0) {
	    ...
	    int cmdl;
	    unpack(buf, "h", &cmdl);
	    process_socket_actions(cmdl, buf);
	  } // send
	} // not a listener
...
void process_socket_actions(int cmdl, unsigned char *buf)
{
  switch(cmdl)
  {
    case REGISTER:
      printf("new client registering\n");
      break;
  }
}
...
void executeCommand(char *userInput)
{
  ...
  for(i=0; i<CMDCOUNT; i++)
  {
	...
	case DISPLAY:
	  return;
	case REGISTER:
	  packetsize = pack(buf, "hs", currentCommand, "Hello Wrodl!");
